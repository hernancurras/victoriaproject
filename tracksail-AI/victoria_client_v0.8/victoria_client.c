...
int calculate_rudder_pos(int new_rudder_pos, int heading_error)
{
	/*Definimos un sector de 10º de |NAVERROR|*/
    if(abs(heading_error)<NAVERROR) 
		{
		new_rudder_pos=0;
		}
	else
		{
		new_rudder_pos = (int)(heading_error * PGAIN); /*Calcular la posicion del timón*/
		}
	
	/*Ajustamos los EPA (End-Point-Adjustment) [RUDDERMIN;RUDDERMAX]*/
	...
	if(new_rudder_pos<=0)
        {
		new_rudder_pos = (-1*new_rudder_pos);
        }
	else
        {
		new_rudder_pos = (360-new_rudder_pos);
        }
...
int main(int argc,char **argv)
{
    ...
    /*GPS*/
	distance = get_wp_distance(distance);   /*distancia al proximo waypoint en metros*/
    desired_heading = get_desired_heading(); 	/*el rumbo al siguiente waypoint*/

	/*NAV*/
    desired_heading = check_tacking(wind_dir,heading,desired_heading); /*Revisamos si deberemos ceñir*/
	heading_error = get_hdg_diff(heading,desired_heading); /*definimos la diferencia entre el rumbo deseado y el rumbo*/
	new_rudder_pos = calculate_rudder_pos(new_rudder_pos,heading_error); /*Calcular la posición del timon*/
	sail_pos = calculate_sail_pos(wind_dir,new_sail_pos); 	/*Calcular la posición de las velas*/
	
	/*Actuators*/
	set_simulator_rudder(new_rudder_pos);
	set_simulator_sail(sail_pos);
	
	/*Telemetry*/
	printf("Id:%d Distancia:%dm Error Rumbo:%d Compass:%d Viento:%d Nueva Posicion Timon:%d Escotas:%d SAILABLE:%d\n",id,distance,heading_error,heading,wind_dir,new_rudder_pos,sail_pos,SAILABLE);
	
	usleep(LAPSE); /*Ciclo de 4 Hz*/
	stop_running = abort;
	id++;
    }
